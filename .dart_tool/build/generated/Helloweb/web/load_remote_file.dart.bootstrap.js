...
let modulePaths = {
 ...
 "web/load_remote_file": "load_remote_file.sound.ddc"
};
if(!window.$dartLoader) {
   window.$dartLoader = {
     appDigests: _currentDirectory + 'load_remote_file.digests',
...
define("load_remote_file.dart.bootstrap", ["web/load_remote_file", "dart_sdk"], function(app, dart_sdk) {
  ...
  (app.web__load_remote_file || app.load_remote_file).main();
  var bootstrap = {
      hot$onChildUpdate: function(childName, child) {
        ...
        if (childName === "load_remote_file.dart") {
          // Clear static caches.
  ...
  dart_sdk.dart.trackLibraries("load_remote_file.dart.bootstrap", {
    "load_remote_file.dart.bootstrap": bootstrap
  }, '');
  return {
    bootstrap: bootstrap
  };
